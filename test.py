...
#            # ==== Total loss ==================================
#            if args.integral:
#                loss = loss_hdr + loss_reg + 0.01 * loss_tv + 0.001 * loss_p
#            else:
...
#            bb = random.choice(brackets)
            ...
            img = cv.imread(ldr_name, -1)[:,:,::-1]
            img = img.transpose(2,0,1) / 255.0
            img = torch.Tensor(img).cuda()
            exposure = torch.LongTensor(ldr_name.split('_')[-2]).reshape(ldr.shape[0],1)
            exposure = torch.zeros(args.batch_size,20).scatter_(1,exposure/50-1,1)
            exposure = exposure.cuda()
            
            with torch.no_grad():
                hdr_pred, label_pred = net(img, exposure)
            
            cv.imwrite(f'output/{exp_name}/{filename.split('/')[-1][:-4]}_pred.hdr', hdr_pred.cpu().numpy().transpose(1,2,0)[:,:,::-1])
