...
def main(args):
    ...
    if args.diff_domain:
        ...
        frames = frames[:3257] # the rest are for testing
        # Split data into training/validation sets
        splitPos = len(frames) - 257
...
#            bb = random.choice(brackets)
    ...
    if args.pretrain == True:
        ...
        state = torch.load(f'checkpoint/{pretrain_name}_best-model.pth.tar')
        net = state['net']
...
#        net.load_state_dict(checkpoint['net'])
        ...
        if args.restore:
            state = torch.load(f'checkpoint/{exp_name}_best-model.pth.tar')
            net = state['net']
